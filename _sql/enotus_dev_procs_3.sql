...
BEGIN
    ...
    IF (var_active_user_id < 0) THEN
        ...
        SELECT '{"error_code":-1, "error_text":"Token not found"}' AS `answer`;
    ...
    ELSE /* If user is valid */
		...
		IF (in_where = '') THEN
			SELECT CONCAT ('{"inserted_id":', LAST_INSERT_ID() ,'}') AS `answer`;
		ELSE
			SELECT CONCAT ('{"row_count":', ROW_COUNT(), '}') AS `answer`;
        END IF;
...
BEGIN
    ...
    IF (in_email = '') THEN
        RETURN '{"error_code":-1, "error_text":"Email not found"}';
    END IF;
    ...
    IF (var_user_id IS NOT NULL) THEN
        ...
        RETURN CONCAT('{"confirm_token":"',var_pass_code,'"}');
    ELSE
        RETURN '{"error_code":0, "error_text":"User not found"}';
    END IF;
